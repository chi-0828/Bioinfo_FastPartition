...
void Kernel::partition(){    
    if(reads_on_SNP.at(positions->at(snp_iterator)).size() > 15){
        cout<<"reads > 15\n";
        exit(1);
    }

    for(unsigned int partition_iterator=0;partition_iterator < reads_on_SNP.at(positions->at(snp_iterator)).size()+1;++partition_iterator){   
        ...
        unsigned int The_read_on_SNP=0;
...
void Kernel::assemble_read(const int pro,int pre){
    ...
    // start assemble
    for(unsigned int i=0;i<previous_partition[pre][0].size();++i){
    ...
    for(unsigned int i=0;i<previous_partition.at(pre).at(1).size();++i){
        // ignore read that has ended
    ...
    //final -> sort
    //cout<<current_partition[pro][0].size()<<" "<<current_partition[pro][1].size()<<"\n";
...
int Kernel::find_match_encode(int currentID , uint64_t encode){
    int match_id = -1; 
    auto min_movement = 10000000;  //to find min cost
    int inverse = 0;
...
void Kernel::build_table(int flag){
    ...
    while (i < current_partition.size()){
        ...
        //build encode
        for(unsigned int k=0;k<same.size();k++){
...
void Kernel::using_best_answer_separate_two_group() {
    ...
    for(unsigned int snp_id = 0;snp_id <snp_size -1;++snp_id){
        ...
        for(unsigned int read_id=0;read_id<reads_on_SNP.at(positions->at(snp_id)).size();++read_id){
            //cout<<read_id<<endl;
...
void Kernel::determine_value_depend_on_group(int &value_a , int &value_b ,int snp_id){
    ...
    for(unsigned int k=0;k<reads_on_SNP.at(positions->at(snp_id)).size();++k){
...
void Kernel::pass_data(){
    ...
    backward_intersect_read = forward_intersect_read; 
    forward_intersect_read.clear();
    snp_iterator++;
}
